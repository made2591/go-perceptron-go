...
func PrepareMLPNet(l []int, lr float64, tf transferFunction, trd transferFunction) (mlp MultiLayerPerceptron) {
	...
	for il, ql := range l {
		...
		if il != 0 {
			...
			// prepare the GENERIC layer with specific dimension and correct number of links for each Neurons
...
// MLPTrain train a mlp MultiLayerPerceptron with BackPropagation algorithm for assisted learning.
func MLPTrain(mlp *MultiLayerPerceptron, stimuli []Stimulus, mapped []string, epochs int) {
